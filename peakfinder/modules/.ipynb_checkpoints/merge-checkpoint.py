...
def visualize_matched_trees(stems):
    """
    Visualize stems for Parzelle 1, where matched stems are green and unmatched stems are red.
    ...
    parzelle_1_stems = [stem for stem in stems if stem[0] == 1]

    # Separate matched and unmatched stems
    matched_stems = [stem for stem in parzelle_1_stems if stem[4] is not None]
    unmatched_stems = [stem for stem in parzelle_1_stems if stem[4] is None]
    ...
    plt.xlabel("UTM_x")
    plt.ylabel("UTM_y")
    plt.title("Visualization of Stems in Parzelle 1")
    plt.legend()
    plt.grid(True)
