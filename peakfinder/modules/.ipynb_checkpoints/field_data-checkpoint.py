...
def find_ground_line(corner_points):
    """
    Identify the lower-left and lower-right points of the ground line based on the start and end of the field measurement.

    Args:
        corner_points (list of tuples): [(Parzelle, Nr, UTM_x, UTM_y), ...]

    Returns:
        tuple: (lower_left, lower_right)
    ...
    sorted_by_y = sorted(corner_points, key=lambda p: p[3])
    lower_left = (sorted_by_y[0][2], sorted_by_y[0][3])

    # Determine the farthest point along the ground line
    # This corresponds to the maximum distance in the X direction (largest UTM_x)
    lower_right = max(corner_points, key=lambda p: p[2])  # Max UTM_x

    return lower_left, (lower_right[2], lower_right[3])
...
def visualize_all_parzellen(corner_points, tree_points):
    """
    Visualizes the corner points and tree points for all Parzellen in a single plot.
    ...
    plt.scatter(corner_x, corner_y, color="blue", label="Parzellen (Corners)", marker="s", s=100)

    # Plot all trees as purple dots
    plt.scatter(tree_x, tree_y, color="purple", label="Trees", alpha=0.7)

    # Labels, legend, and title
    plt.xlabel("UTM_x")
    plt.ylabel("UTM_y")
    plt.title("All Parzellen: Trees and Corners")
    plt.legend()
    plt.grid(True)
    plt.show()
