...
def segment_trees(
    ...
    # Load the nDSM file and compute subset window transform when requested
...
def match_and_visualize_updated(
    ...
    for segment_id, stems in segment_to_stems.items():
        ...
        if len(stems) == 1 and len(treetops) == 1:
            ...
            # Refine the tree height using 98th percentile of segment heights (more robust than max)
            segment_mask = labels == segment_id
            segment_heights = ndsm[segment_mask]
            max_height = float(np.percentile(segment_heights, 98.0))
            
            # Find pixel with closest value to 98th percentile for coordinate refinement
            refined_idx = np.argmin(np.abs(segment_heights - max_height))
            refined_r, refined_c = np.where(segment_mask)[0][refined_idx], np.where(segment_mask)[1][refined_idx]
...
def match_and_visualize_updated2(
    ...
    for segment_id, stems in segment_to_stems.items():
        ...
        if segment_id == "no_segment":
            ...
            # Compute the 98th percentile of segment heights (robust tree height)
            segment_mask = labels == segment_id
            segment_heights = ndsm[segment_mask]
            max_height = float(np.percentile(segment_heights, 98.0))

            # Keep the original treetop pixel (from the watershed markers) for coordinates
            # Convert original treetop pixel coordinates to UTM for reporting
            utm_x_treetop, utm_y_treetop = rasterio.transform.xy(transform, r_treetop, c_treetop, offset="center")

            # Add matched pair to the result list. Tree height comes from 98th percentile,
            # but the treetop coordinates remain the original marker-based treetop.
            result_list.append({
                "Parzelle": parzelle,
                "Nr": nr,
                "Stem_UTM_X": x_stem,
